...
export class DataManager {
  ...
  private async loadItemsCatalog(): Promise<void> {
    const data = await this.loadJSON<{ items: ItemData[] }>('items_catalog.json');
    data.items.forEach(item => {
      this.itemsCatalog.set(item.objectID, item);
    });
  }
  ...
  private async loadWeapons(): Promise<void> {
    ...
    data.items.forEach(spec => {
      const catalog = this.itemsCatalog.get(spec.objectID);
      if (catalog) {
        const itemData = { ...spec, ...catalog };
        this.items.set(spec.objectID, new Item(itemData));
      }
    });
  }
