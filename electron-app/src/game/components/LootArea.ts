...
import type { LootSource, LootSourceType } from '@types';
...
export class LootArea extends PIXI.Container {
  ...
  private gridContainers: Map<string, Subgrid> = new Map();
  ...
  addLootSource(source: LootSource): boolean {
    ...
    // 创建网格容器
    const gridContainer = new Subgrid({
      size: { width: 15, height: 40 },
      cellSize: this.cellSize,
      title: `loot_${source.id}`,
    });
    gridContainer.container.visible = false;
    this.contentContainer.addChild(gridContainer.container);
  ...
  removeLootSource(sourceId: string): boolean {
    ...
    if (gridContainer) {
      this.contentContainer.removeChild(gridContainer.container);
  ...
  switchTab(sourceId: string): void {
    ...
    const gridContainer = this.gridContainers.get(sourceId);
    if (gridContainer) {
      gridContainer.container.visible = true;
    }
